...
def createLinks(dets_all):
    ...
    for cls_ind in range(cls_num):
        ...
        for frame_ind in range(frame_num - 1):
            dets1 = dets_all[cls_ind][frame_ind].bbox.cpu().numpy()
            dets2 = dets_all[cls_ind][frame_ind + 1].bbox.cpu().numpy()
            box1_num = len(dets1)
            box2_num = len(dets2)
            ...
            if frame_ind == 0:
                ...
                areas1 = (dets1[:, 2] - dets1[:, 0] + 1) * (dets1[:, 3] - dets1[:, 1] + 1)
                #areas1 = areas1.cpu().numpy()
            else:
                areas1 = areas2
            ...
            areas2 = (dets2[:, 2] - dets2[:, 0] + 1) * (dets2[:, 3] - dets2[:, 1] + 1)
            #areas2 = areas2.cpu().numpy()
        ...
        while True:
            ...
            for i, box_ind in enumerate(maxpath):
                ...
                for j in delete_set[i]:
                    ...
                    dets_cls[i + rootindex].bbox[j].zero_()  # fill zeros
                    dets_cls[i + rootindex].extra_fields["scores"][j] = 0.
...
def findMaxPath(links, dets, delete_single_box):
    ...
    for l in range(len(dets)):
        for j in range(len(dets[l])):
            if (new_dets[l, j] == 0):
                a[l, j] = dets[l].extra_fields['scores'][j] # dets[l][j][-1]
    ...
    for i in range(1, num_frame):
        ...
        for box_id, box in enumerate(links[l1]):
            for next_box_id in box:
                ...
                weight_new = a[i - 1, box_id] + dets[i].extra_fields['scores'][next_box_id] # dets[i][next_box_id][-1]
...
def rescore(dets, rootindex, maxpath, maxsum):
    ...
    for i, box_ind in enumerate(maxpath):
        ...
        dets[rootindex + i].extra_fields["scores"][box_ind] = newscore
...
def deleteLink(dets, links, rootindex, maxpath, thesh):
    ...
    for i, box_ind in enumerate(maxpath):
        bboxes = dets[rootindex + i].bbox.cpu().numpy()
        areas = [(box[2] - box[0] + 1) * (box[3] - box[1] + 1) for box in bboxes]
        area1 = areas[box_ind]
        box1 = bboxes[box_ind]
        x1 = np.maximum(box1[0], bboxes[:, 0])  # dets[rootindex + i][:, 0])
        y1 = np.maximum(box1[1], bboxes[:, 1])  # dets[rootindex + i][:, 1])
        x2 = np.minimum(box1[2], bboxes[:, 2])  # dets[rootindex + i][:, 2])
        y2 = np.minimum(box1[3], bboxes[:, 3])  # dets[rootindex + i][:, 3])
        w = np.maximum(0.0, x2 - x1 + 1)
        h = np.maximum(0.0, y2 - y1 + 1)
        inter = w * h
