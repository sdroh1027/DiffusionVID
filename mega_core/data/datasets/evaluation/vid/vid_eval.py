...
def do_vid_evaluation(dataset, predictions, output_folder, box_only, motion_specific, logger):
    ...
    corloc, corloc_avg, corloc_info = corloc_eval_detection_vid(
        pred_boxlists=pred_boxlists,
        gt_boxlists=gt_boxlists,
        iou_thresh=0.5
    )
    ...
    result_str += "Mean CorLoc: {:.4f}\n".format(corloc_avg)
    result_str += "Category CorLoc:\n"
    for l in corloc.keys():
        result_str += "{:<16}: {:.4f}\n".format(
            dataset.map_class_id_to_class_name(l), corloc[l]
        )
    logger.info("\n" + result_str)
    if output_folder:
...
def corloc_eval_detection_vid(pred_boxlists, gt_boxlists, iou_thresh=0.5):
    assert len(gt_boxlists) == len(pred_boxlists), "Length of gt and pred lists need to be same."
    print("Evaluating CorLoc")

    gt_imgs = defaultdict(int)
    images_correct = defaultdict(int)
    corloc = defaultdict(int)
    info = ''
    for gt_boxlist, pred_boxlist in zip(gt_boxlists, pred_boxlists):
        pred_bbox = pred_boxlist.bbox.numpy()
        pred_label = pred_boxlist.get_field("labels").numpy()
        ...
        # select only first box
        order_boxlist = pred_score.argsort()[::-1]
        first_boxlist = order_boxlist[:1]
        pred_bbox = pred_bbox[first_boxlist]
        pred_label = pred_label[first_boxlist]
        pred_score = pred_score[first_boxlist]

        for l in gt_label.astype(int):
            # mask pred box by label
            pred_mask_l = pred_label == l
            pred_bbox_l = pred_bbox[pred_mask_l]
            pred_score_l = pred_score[pred_mask_l]
            ...
            #pred_score_l = pred_score_l[order]
            first = order[:1]
            pred_bbox_l = pred_bbox_l[first]
            pred_score_l = pred_score_l[first]

            # mask gt box by label
            gt_mask_l = gt_label == l
            gt_bbox_l = gt_bbox[gt_mask_l]
            gt_ignore_l = gt_ignore[gt_mask_l]
    ...
    num_images_correctly_detected_per_class = 0
    corloc_sum = 0

    for l in gt_imgs.keys():
        info += 'gt:{} correct:{}\n'.format(gt_imgs[l], images_correct[l])
        corloc[l] = compute_cor_loc(
            gt_imgs[l],
            images_correct[l],
        )
        num_gt_imgs_per_class += gt_imgs[l]
        num_images_correctly_detected_per_class += images_correct[l]
        corloc_sum += corloc[l]

    info += "total_gt:{}, total_correct:{}\n".format(num_gt_imgs_per_class, num_images_correctly_detected_per_class)
    corloc_total = compute_cor_loc(
        num_gt_imgs_per_class,
        num_images_correctly_detected_per_class,
    )
    print(corloc)
    corloc_avg = corloc_sum / len(gt_imgs.keys())
    info += "Avg_CorLoc = {:.4f}".format(corloc_avg)
    return corloc, corloc_avg, info
